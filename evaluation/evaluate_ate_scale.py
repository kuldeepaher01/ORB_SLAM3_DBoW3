...
def align(model, data):
    ...
    for column in range(data_zerocentered.shape[1]):
        ...
        norms += np.linalg.norm(model_zerocentered[:, column])**2
...
def plot_traj(ax, stamps, traj, style, color, label):
    ...
    for i in range(len(stamps)):
        if stamps[i] - last < 2 * interval:
            ...
            ax.plot(x, y, style, color=color, label=label, linewidth=3)
            label = ""
            x = []
            y = []
        last = stamps[i]
...
if __name__ == "__main__":
    ...
    second_xyz_full = np.matrix([[float(value) * float(args.scale) for value in sorted_second_list[i][1][0:3]] for i in range(len(sorted_second_list))]).transpose()
    ...
    if args.verbose:
        ...
        print(f"absolute_translational_error.rmse {np.sqrt(np.dot(trans_error, trans_error) / len(trans_error))} m")
        print(f"absolute_translational_error.mean {np.mean(trans_error)} m")
        print(f"absolute_translational_error.median {np.median(trans_error)} m")
        print(f"absolute_translational_error.std {np.std(trans_error)} m")
        print(f"absolute_translational_error.min {np.min(trans_error)} m")
        print(f"absolute_translational_error.max {np.max(trans_error)} m")
        print(f"max idx: {np.argmax(trans_error)}")
    else:
        print(f"{np.sqrt(np.dot(trans_error, trans_error) / len(trans_error))},{scale},{np.sqrt(np.dot(trans_errorGT, trans_errorGT) / len(trans_errorGT))}")
    
    if args.verbose2:
        print(f"compared_pose_pairs {len(trans_error)} pairs")
        print(f"absolute_translational_error.rmse {np.sqrt(np.dot(trans_error, trans_error) / len(trans_error))} m")
        print(f"absolute_translational_errorGT.rmse {np.sqrt(np.dot(trans_errorGT, trans_errorGT) / len(trans_errorGT))} m")
